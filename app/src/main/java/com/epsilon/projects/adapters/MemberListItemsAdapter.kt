...
open class MemberListItemsAdapter(
    ...
    private var onClickerListener: OnClickListener? = null
        ...
        if (holder is MyViewHolder) {
            ...
            holder.itemView.setOnClickListener{
                if(onClickerListener != null){
                    if(model.selected){
                        onClickerListener!!.onClick(position,model, Constants.UN_SELECT)
                    }else{
                        onClickerListener!!.onClick(position,model, Constants.SELECT)
                    }
                }
            }
        }
